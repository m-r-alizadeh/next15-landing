            ...
            <div className="py-24 px-4 overflow-hidden">
                ...
                <div className="container max-w-7xl mx-auto">
                    <div className=" flex flex-col gap-8">
                        ...
                        <h2 className="text-4xl md:text-6xl font-medium text-center max-w-xl mx-auto">
                            {t("heading1")}
                            <span className="text-red-400 ml-2 block ">{t("heading2")}</span>
                        </h2>
                        <div className="flex flex-col gap-6 max-w-xl mx-auto">
                            {Array.from({ length: faqCount - 1 }).map((_, index) => (
                                <div key={index} className="bg-neutral-900 rounded-2xl border border-white/10 p-6 "
                                    onClick={() => setSelectedIndex(index)}>
                                    <div className="flex items-center justify-between cursor-pointer">
                                        <h3 className="font-medium">{t(`item${index + 1}.question`)}</h3>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                                            className={cn("feather feather-plus text-red-400 flex-shrink-0 transition duration-300", selectedIndex === index && 'rotate-45')}>
